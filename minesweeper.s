...
#
#
# This program was written by Mubtasim Masud (z5362052)
# on Start date: 22/10/2021
#
...
reveal_grid__body:
        ...
        li      $t8, 0                  # int row = 0;
for_each_row:
        bge     $t8, N_ROWS, reveal_grid__epilogue   #if (row >= N_ROWS) goto reveal_grid__epilogue:;
        li      $t9, 0                  # int col = 0; 
for_each_column:
        bge     $t9, N_COLS, next_row   #if (row >= N_COLS) goto finished;
        la      $t0, grid               #getting the address of the grid
        mul     $t1, $t8, N_COLS        #figuring out the size of each row in memory
        add     $t3, $t0, $t1           #travelling to the current row in the grid and putting that address in t3
        add     $t4, $t3, $t9           # travelling to the current column in the row and putting that address in t4
        lb      $t5, ($t4)              #loading the byte that is in t4 into t5
        or      $t5, $t5, IS_RVLD_MASK  # t5 |= IS_RVLD_MASK
        sb      $t5, ($t4)              #sending the value in t5 to the address in t4
        addi    $t9, $t9, 1             #col++;
...
place_bombs_for:
        bge     $s5, $s6, bomb_limit_reached

        addi    $sp, $sp, -12
        sw      $ra, 0($sp)
        sw      $a0, 4($sp)       #saving ra, and first two argument registers
        sw      $a1, 8($sp)

        jal     place_single_bomb  #calling place_single_bomb function

        lw      $ra, 0($sp)
        lw      $a0, 4($sp)        #retrieving ra, and first two argument registers
        lw      $a1, 8($sp)
        addi    $sp, $sp, 12
...
mark_cell__body:
        ...
        # address of grid in $t0
        #$t1, $t2 $t3 hold temporary values
        lb      $t5, debug_mode
        
        la      $t0, grid
        mul     $t1, $a0, N_COLS #getting row number
        add     $t1, $t1, $t0    #getting the address to the right row
        add     $t1, $t1, $a1    #getting to the right column in the right row
        lb      $t3, ($t1)       #loading the cell

        and     $t4, $t3, IS_RVLD_MASK  #if (grid[row][col] & IS_RVLD_MASK) 
        bne     $t4, 0, revealed_cell   #       goto revealed_cell
        and     $t4, $t3, IS_MRKD_MASK  #if (grid[row][col] & IS_MRKD_MASK) 
        bne     $t4, 0, unmarking_cell  #       goto unmarking_cell
marking_cell:
        or      $t3, $t3, IS_MRKD_MASK # grid[row][col] |= IS_MRKD_MASK;
        sb     $t3, ($t1)              # sending the marked cell to it's rightful address
        lb     $t7, bomb_count         #
        addi   $t7, $t7, -1            #decrementing bomb count
        sb     $t7, bomb_count
...
unmarking_cell:
        li     $t7, IS_MRKD_MASK
        not    $t6, $t7         #getting the not value of IS_MRKD_MASK
        and    $t3, $t3, $t6    #then "&="ing it with grid[row][col]
        sb     $t3, ($t1)
        lb     $t7, bomb_count   #
        addi   $t7, $t7, 1       #bomb_count++       
        sb     $t7, bomb_count   #
        j mark_cell__epilogue
revealed_cell:
        bne     $t5, 0, mark_cell__epilogue #checking for debug mode
        la      $a0, mark_error  #printing relevant string
        li      $v0, 4
        syscall
...
reveal_cell__body:
        ...
        la      $t0, grid        #address of grid in $t0
        mul     $t1, $a0, N_COLS #getting row number
        add     $t1, $t1, $t0    #getting the address to the right row
        add     $t1, $t1, $a1    #getting to the right column in the right row
        lb      $t2, ($t1)       #loading the cell


        and     $t4, $t2, IS_MRKD_MASK
        bne     $t4, 0, already_marked  # if (grid[row][col] & IS_MRKD_MASK) goto already_marked

        and     $t4, $t2, IS_RVLD_MASK  # if (grid[row][col] & IS_RVLD_MASK) goto already_revleaed 
        bne     $t4, 0, already__revealed

        and     $t4, $t2, IS_BOMB_MASK  # if (grid[row][col] & IS_RVLD_MASK) goto bomb_triggered
        bne     $t4, 0, bomb_triggered

        j       revealling_cell

already_marked:
        lw      $t4, debug_mode
        bne     $t4, 0, reveal_cell__epilogue   #checking if debug mode is 1
        la      $a0, reveal_error  #printing relevant string
        li      $v0, 4
        syscall
...
already__revealed:
        lw      $t4, debug_mode
        bne     $t4, 0, reveal_cell__epilogue #checking if debug mode is 1
        la      $a0, already_revealed  #printing relevant string
        li      $v0, 4
        syscall
        j	reveal_cell__epilogue	# jump to reveal_cell__epilogue
bomb_triggered:
        li      $t4, LOSE
        sw      $t4, game_state #Send LOSE to the address labelled game_state
        j	revealling_cell	
...
clear_surr:
        ...
        j       check_for_win
revealling_cell:        
        and     $t5, $t2, VALUE_MASK    # if ((grid[row][col] & VALUE_MASK) == 0)  goto clear_surr                                                         
        beq     $t5, 0, clear_surr         
        or      $t2, $t2, IS_RVLD_MASK #grid[row][col] |= IS_RVLD_MASK;
        sb      $t2, 0($t1)

if_not_lose:
        lw      $t6, game_state
        bne     $t6, LOSE, decrement_cells #check the game_state, if it's not LOSE, decrement cells
        j       check_for_win
decrement_cells:
        lw      $t7, cells_left
        addi	$t7, $t7, -1     #cells_left--;
        sw      $t7, cells_left			
check_for_win:
        lw      $t7, cells_left
        beq     $t7, 0, game_won ##check cells_left, if it's zero, goto game_won
        j	reveal_cell__epilogue
game_won:
        li      $t8, WIN
        sw      $t8, game_state #update the game state with WIN
        j	reveal_cell__epilogue	# jump to reveal_cell__epilogue
...
clear_surroundings__body:
        ...
        bge     $a0, N_ROWS, clear_surroundings__epilogue       # if (row < 0 || row >= N_ROWS || col < 0 || col >= N_COLS)
        blt     $a1, 0, clear_surroundings__epilogue            #       return;
        ...
        li      $t3, IS_RVLD_MASK
        and     $t4, $t2, $t3                           #if (grid[row][col] & IS_RVLD_MASK) return;
        bne     $t4, 0, clear_surroundings__epilogue

        or      $t5, $t2, IS_RVLD_MASK
        sb      $t5, ($t1)              #grid[row][col] |= IS_RVLD_MASK;
        lw      $t6, cells_left         #cells_left--;
        addi    $t6, $t6, -1
        sw      $t6, cells_left

        li      $t7, IS_MRKD_MASK
        not     $t7, $t7
        lb      $t2, ($t1)              #   grid[row][col] &= ~IS_MRKD_MASK;
        and     $t2, $t2, $t7
        sb      $t2, ($t1)

        lb      $t2, ($t1)
        and     $t2, $t2, VALUE_MASK    #if (grid[row][col] & VALUE_MASK) return;
        bne     $t2, 0, clear_surroundings__epilogue

        move    $s0, $a0
        move    $s1, $a1        #saving coordinates in s registers in order to preserve the information in them

        add     $a0, $s0, -1
        move    $a1, $s1
        jal     clear_surroundings      #each of these blocks are recursive function 
                                      #calls that are in the exact same order as 
                                        #in the C code above
        add     $a0, $s0, -1
        add     $a1, $s1, -1
...
update_highscore__body:
        ...
        la      $t0, high_score
        lw      $t1, 0($t0)            #retrieve the high score struct

        bge     $t1, $a0, not_high_enough #if it's bigger than or equal to the current user score then return FALSE
        sw      $a0, 0($t0)               
        
        add     $t2, $t0, 4 #address of high_score.name
        la      $t8, user_name
        li      $t3, 0      #int i = 0

        
while_username:         #individually copy each char of the new high scorer into the high score struct
        add     $t4, $t2, $t3
        add     $t9, $t8, $t3
        lb      $t5, 0($t9)    
        sb      $t5, 0($t4)
...
copy_complete:
        ...
        sb      $0, 0($t4)      #add a null terminator at the end
...
print_scores__body:
        ...
        la      $a0, scores_msg  #printing relevant string
        li      $v0, 4
        syscall
        ...
        la      $t1, scores
for_each_score:
        bge     $t0, MAX_SCORES, scores_finished_printing
        mul     $t2, $t0, USER_SCORE_SIZE
        add     $t3, $t2, $t1

        lw      $t4, ($t3)
        beq     $t4, -1, scores_finished_printing

        
        la      $a0, scores_line_msg  #printing relevant string
        li      $v0, 4
        syscall
        la      $a0, scores_username_msg  #printing relevant string
        li      $v0, 4
        syscall
        la      $a0, 4($t3)  #printing relevant string
        li      $v0, 4
        syscall
        ...
        la      $a0, scores_score_msg #printing relevant string
        li      $v0, 4
        syscall
